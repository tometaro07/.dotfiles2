...
local PATH_PLUGIN = config.ConfigDir .. "/plug/bookmark/bookmarks/"
...
function _init_bookmark_table(bn)
	local bookmarks = {
	...
	local filename = bn:match("[^/]*$")
	local file = io.open(PATH_PLUGIN..filename..".marks","r")
	if file~=nil then
		io.input(file)
		local i = 1
		for line in io.lines() do
			table.insert(bookmarks.marks,tonumber(line))
			i = i+1
		end
		io.close(file)
	end
...
function _save_file(bp)
	local bn = bp.Buf:GetName()
	
	local filename = bn:match("[^/]*$")
	if #bd[bn].marks > 0 then
		-- Save file
		local file = assert(io.open(PATH_PLUGIN..filename..".marks","w+"))
		io.output(file)
		io.write(table.toString(bd[bn].marks))
		io.close(file)
	else
		os.remove(PATH_PLUGIN..filename..".marks")
	end
end
